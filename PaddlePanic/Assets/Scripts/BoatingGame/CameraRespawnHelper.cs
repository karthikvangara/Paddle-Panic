...
public class CameraRespawnHelper : MonoBehaviour
{
    ...
    {
        Debug.Log("Camera Handling when respawned");
        // Move the boat instantly
        boat.transform.position = respawnPoint.position;
        boat.transform.rotation = respawnPoint.rotation;

        // Force Cinemachine to snap to new position (no smoothing)
